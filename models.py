...
from preprocessing_improvements import ImprovedPreprocessor, apply_preprocessing_to_model_pipeline
...
def train_models(df, _loader):
    ...
    try:
        corr_matrix = df[feature_cols + ['owners']].corr()
...
def train_improved_models(df, feature_cols):
    ...
    if 'release_date' in df.columns:
        ...
        X_train, X_test, y_train_log, y_test_log = train_test_split(
            X, y_owners_log, test_size=0.2, random_state=42
        )
        _, _, y_train_actual, y_test_actual = train_test_split(
    ...
    training_log['preprocessing']['stratification'] = 'None (temporal split)'
    ...
    # Ranking metrics for percentile validation
    from scipy.stats import spearmanr

    print("\n  📊 Ranking Accuracy (for percentile positioning):")
    ...
    for pct in [80, 90, 95]:
        threshold_actual = np.percentile(y_test_actual, pct)
        threshold_pred = np.percentile(owners_pred, pct)
        actual_top = y_test_actual >= threshold_actual
        pred_top = owners_pred >= threshold_pred
        accuracy = np.mean(actual_top == pred_top)
...
def generate_training_report(df, feature_cols, results, training_time):
    ...
    report_lines.append("for predicting game success metrics (owner count) based on Steam game data.")
    report_lines.append("")
    ...
    report_lines.append("7. **Ratio Features:** Computed meaningful ratios (e.g., playtime_skewness, rating_ratios)")
    report_lines.append("8. **Interaction Terms:** Combined features (e.g., price × quality, age × engagement)")
    report_lines.append("9. **Domain Features:** Created game-specific metrics (e.g., is_free, is_mature, achievement_tiers)")
    report_lines.append("")
    ...
    report_lines.append(f"- **Models Trained:** 1 (Owners)")
    report_lines.append("")
    ...
    # Add ranking metrics section
    report_lines.append("### 5.2 Ranking Accuracy (Percentile Positioning)")
    report_lines.append("")
    report_lines.append("These metrics validate how well the model ranks games relative to each other,")
    report_lines.append("which is critical for percentile-based positioning.")
    report_lines.append("")
    report_lines.append("| Metric | Value | Interpretation |")
    report_lines.append("|--------|-------|----------------|")
    ...
    if 'spearman_correlation' in metrics:
        ...
        spearman_interp = "Excellent" if spearman >= 0.80 else "Good" if spearman >= 0.60 else "Moderate" if spearman >= 0.40 else "Poor"
        report_lines.append(f"| **Spearman Correlation** | {spearman:.4f} | {spearman_interp} ranking agreement |")

    if 'mae_percentile' in metrics:
        mae_pct = metrics['mae_percentile']
        pct_interp = "Excellent" if mae_pct < 5 else "Good" if mae_pct < 10 else "Moderate" if mae_pct < 15 else "Poor"
        report_lines.append(f"| **MAE in Percentiles** | {mae_pct:.2f} points | {pct_interp} - avg error in percentile assignment |")

    report_lines.append("")
    report_lines.append("**Key Insight:** Even if absolute predictions have moderate R² (~0.36), high ranking")
    report_lines.append("correlation (Spearman > 0.60) means percentile-based positioning is still reliable.")
    report_lines.append("")
    ...
    report_lines.append("predicting game owner counts. The model effectively captures patterns in game success, ")
    report_lines.append("with particular strength in ranking games relative to each other (percentile positioning).")
    report_lines.append("")
